            ...
            <div class="row">
                <div class="col-md-8">
                    <article class="post">
                        ...
                        <div class="post-content">
                            ...
                            <div class="social-share">
                                <span class="social-share-title pull-left text-capitalize">By {{ $post->author ? $post->author->name : 'No Name'}} On {{ $post->getDate() }}</span>
                    ...
                    <div class="top-comment"><!--top comment-->
                        <img src="{{ $post->author ? $post->author->getImage() : 'No Name'}}" width="100" class="pull-left img-circle" alt="">
                        <h4>{{ $post->author ? $post->author->name : 'No Name'}}</h4>

                        <p>{{ $post->author ? $post->author->about_me : 'No Name'}}.</p>
                    </div><!--top comment end-->
                    <div class="row"><!--blog next previous-->
                        <div class="col-md-6">
                            @if($post->hasPrevios())
                                <div class="single-blog-box">
                                    <a href="{{ route('post.show', $post->getPrevios()->slug) }}">
                                        <img src="{{ $post->getPrevios()->getImage() }}" alt="">
                                        <div class="overlay">
                                            <div class="promo-text">
                                                <p>
                                                    <i class=" pull-left fa fa-angle-left"></i>
                                                </p>
                                                <h5>{{ $post->getPrevios()->title }}</h5>
                                            </div>
                                        </div>
                                    </a>
                                </div>
                        ...
                        @foreach($post->getComments() as $comment)
                            <div class="bottom-comment"><!--bottom comment-->
                                <div class="comment-img">
                                    <img class="img-circle" width="75" height="75" src="{{ $comment->author->getImage() }}" alt="">
                                </div>
                ...
                <!-- end bottom comment-->
                    @if(Auth::check())
                        <div class="leave-comment"><!--leave comment-->
                            <h4>Leave a reply</h4>
                            <form class="form-horizontal contact-form" method="post" action="/comment">
                                {{ csrf_field() }}
